...
const (
	Format_ISO8601  Format = "2006-01-02"
	Format_FACEBOOK Format = "01/02/2006"
)

var DateFormat Format = Format_ISO8601
...
// FromString creates a date from its Format_ (YYYY-MM-DD) representation.
func FromString(str string) (Date, error) {
	t, err := time.Parse(string(DateFormat), str)
	if err != nil {
		return 0, err
	}
...
// MustFromString creates a date from its Format_ (YYYY-MM-DD) representation.
...
// String returns the Format_ representation (YYYY-MM-DD).
...
// MarshalJSON marshals the date into a JSON string in Format_ format.
func (d Date) MarshalJSON() ([]byte, error) {
	return []byte(`"` + d.String() + `"`), nil
}

// UnmarshalJSON unmarshals a JSON string in the Format_ format into a date.
